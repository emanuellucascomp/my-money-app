...
class CreditList extends Component {
    ...
    add(indice, item={}){
        if(!this.props.readOnly){
            this.props.arrayInsert('billingCycleForm', 'credits', indice, item);
        }
    }

    remove(indice){
        if(!this.props.readOnly && this.props.list.length > 1){
            this.props.arrayRemove('billingCycleForm', 'credits', indice);
        }
    }
        ...
        return list.map((item, index) => (
            <tr key={index}>
                ...
                <td>
                    ...
                    <button type='button' className='btn btn-danger' onClick={() => this.remove(index)}><i className='fa fa-trash-o'></i></button>
                </td>
            </tr>
        ));
    }
